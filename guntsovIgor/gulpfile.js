...
const webpackConfig = {
    ...
    watch: false,
    devtool: isDev ? "source-map" : "none", //'none'
    module: {
        rules: [
            {
                test: /\.m?js$/,
...
gulp.task("build", gulp.parallel("html:build", "js:build", "style:build", "images:build", "fonts:build"));
...
gulp.task("prod", gulp.series("clean", gulp.parallel("build"))); //надо запустить если isDev : false
//gulp if разобраться через require установить и закинуть всередину pipe(sourceMap.init())
// в девелопер мод должны быть сурсмапы..?
